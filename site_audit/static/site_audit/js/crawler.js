...
// function for update progressbars
function UpdateBars() {
...
// console.log("hhh", generalPassedScore, generalFailedScore, generalWarningScore);
  var health =
...
//  console.log("mu health", health)
 $("#siteHealth_radiall").html("");
  if (health !== "") {
    // console.log("h1")
    ...
    if (health !== undefined) {
      ...
      // console.log("h5", health >= yellow_lower_range && health <= yellow_upper_range, yellow_lower_range, yellow_upper_range, health)
      if (health >= yellow_lower_range && health <= yellow_upper_range) {
        ...
        site_health_radial = new ApexCharts(
          document.querySelector("#siteHealth_radiall"),
          options_ave
        );
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if (response.status === true ) {
        $("#tb_loader").hide();
        total_crawl_pages_count = response.total_crawl_pages_count;
      //  total_crawl_pages_count = 9
       total_webpages_count = response.allowed_pages;
      //  total_webpages_count = 10
       console.log("resssss", total_crawl_pages_count, total_webpages_count)
  // crawler page start
  ...
  if(percentage_count){
    ...
    $(".pages_crawled_circle").circleProgress({
      ...
      fill: {
        ...
        const main_url = response.crawl_pages.main_page;
        const page_title = response.crawl_pages.page_title;
        console.log(main_url,'main_url')
         $(".accordionLink").empty()
        if(main_url !== '' && main_url !== undefined){
         
         $('.titleTag').html(page_title)
         $(".accordionLink").attr("title", page_title)
         $(".accordionLink").attr("href", main_url);
         $(".accordionLink").html(main_url); 
        }else{
         $('.titleTag').html('--')
         $(".accordionLink").attr("title", '--')
         $(".accordionLink").attr("href",'javascript:void()');
         $(".accordionLink").html('--');
        }
       
        var inner_pages = response.crawl_pages.inner_pages;
        console.log(inner_pages,'inner_pages')
        $(document).ready(function () {
        ...
        $.each(inner_pages, function (i, val) {
         ...
         <div class="row gy-md-0 gy-2">
            ...
            <div class="col-lg-3">
                <p class="text-gray text-left mb-0 font-12 text-elipses text-capitalize  "title="${val.page_title}">
                     ${val.page_title}
                </p>
            </div>
            <div class="col-lg-2">
        ...
        $(".cls").each(function () {
          ...
          $("#selectedDomain > option").each(function () {
            if (url_attr === $(this).val() || url_attr.replace(/\/$/, "") === $(this).val().replace(/\/$/, "")) {
              ...
              // $(`#${url_id}`).closest('.custom-tbody').hide();
            }
          });
        });
        //  $('.cls').each(function(){
...
$(document).on("change", ".radio", function () {
  $("#siteHealth_radiall").empty()
  $("#siteHealth_radiall").hide()
  $("#finalUrl").html("");
  $("#dateTim").html("");
  //   progressBar(0, 0);
  $(".failed_val").html(`${0}`);
  $(".warning_val").html(`${0}`);
  $(".passed_val").html(`${0}`);
  $(".gn.progress-bar.bg-fail").css("width", 8 + "%");
  $(".gn.progress-bar.bg-warn").css("width", 8 + "%");
  $(".gn.progress-bar.bg-pass").css("width", 8 + "%");
    ...
    if(response.status){
      response.data.map((val, id) => {
        if(val.plan_id_id == 'Free Plan') {
          $("#unlock-btn").attr("href", "/subscription/upgrade_subscription/");
          $("#unlock-btn").css("background-color", "#eb4335")
          $("#success_lock").attr("src","/static/site_audit/assets/img/lock.png");
        }
        else {
          $("#unlock-btn").css("background-color", "#8bc34a").prop("disabled", true);
          $("#success_lock").attr("src", "/static/site_audit/assets/success_unlock.png");
        }
      });
    }
  }
    ...
    if(response.status){
      ...
      response.data.map((val, id) => {
        if(val.plan_id_id == 'Free Plan' || val.plan_id_id == 'Pro Plan' ) {
          $("#unlock-btn").attr("href", "/subscription/upgrade_subscription/?unlock");
          $("#unlock-btn").css("background-color", "#eb4335")
          $("#success_lock").attr("src","/static/site_audit/assets/img/lock.png");
        }
        else {
         ...
         if(val.plan_id_id == 'Free Plan' || val.plan_id_id == 'Pro Plan') {
          $(".crawlerFreeProSection").show()
         }else{
          $(".crawlerFreeProSection").hide()
         }
